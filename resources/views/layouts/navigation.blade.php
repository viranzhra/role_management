<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @if (auth()->check() && auth()->user()->can('tes jurusan'))
                        <x-nav-link :href="route('ujian.index')" :active="request()->routeIs('ujian.index')">
                            {{ __('Tes Ujian') }}
                        </x-nav-link>
                    @endif
                    @if (auth()->user()->can('view kelas') ||
                            auth()->user()->can('edit kelas') ||
                            auth()->user()->can('create kelas') ||
                            auth()->user()->can('delete kelas'))
                        <x-nav-link :href="route('kelas.index')" :active="request()->routeIs('kelas.index')">
                            {{ __('Data Kelas') }}
                        </x-nav-link>
                    @endif
                    @if (auth()->user()->can('view siswa') ||
                            auth()->user()->can('edit siswa') ||
                            auth()->user()->can('create siswa') ||
                            auth()->user()->can('delete siswa'))
                        <x-nav-link :href="route('siswa.index')" :active="request()->routeIs('siswa.index')">
                            {{ __('Data Siswa') }}
                        </x-nav-link>
                    @endif
                    @if (auth()->check() && auth()->user()->can('view aduan'))
                        <x-nav-link :href="route('pengaduan.index')" :active="request()->routeIs('pengaduan.index')">
                            {{ __('Aduan Management') }}
                        </x-nav-link>
                    @endif
                    @if (auth()->check() && auth()->user()->can('create aduan'))
                    ...
                    <x-nav-link :href="route('jenisMasalah.index')" :active="request()->routeIs('jenisMasalah.index')">
                        {{ __('Jenis Masalah') }}
                    </x-nav-link>
                    <x-nav-link :href="route('kehadiran.index')" :active="request()->routeIs('kehadiran.index')">
                        {{ __('Absensi Harian') }}
                    </x-nav-link>
                    @if (auth()->check() && auth()->user()->can('view kehadiran'))
                        <x-nav-link :href="route('kehadiran.getData')" :active="request()->routeIs('kehadiran.getData')">
                            {{ __('Data Daftar Hadir') }}
                        </x-nav-link>
                    @endif
    ...
    <div :class="{ 'block': open, 'hidden': !open }" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <x-responsive-nav-link :href="route('role-management.index')" :active="request()->routeIs('role-management.index')">
                {{ __('Role Management') }}
            </x-responsive-nav-link>
        </div>
